...
weight on talent relative to social capital. Second, this ceiling
...
expenditure in China \citep{JinLiWu2011} and cross-national data
\citep{ChaiKausKiedaisch2019} providing systematic documentation.
...
human capital they embody. In the Weberian tradition of
status-group closure \citep{WeberWirtschaft1922}, formalised as a
theory of credentialism by \citet{Collins1979}, credentials confer
legitimate claim to a social position by controlling access to
positional goods through institutional requirements, not by raising
productivity. Rising inequality raises both the stakes of winning a
...
education expenditure and inequality move together in the data. The
...
feasible at all. The threshold cost structure that makes
participation a discrete binary decision --- affordable or not,
depending on income --- is absent from this literature. Making
participation subject to a threshold budget constraint --- the
single architectural change in the present paper --- breaks the
symmetry: wider income gaps simultaneously strengthen the
...
and status expenditure then
follows as the generic equilibrium outcome, not a special case.
...
human capital framework does not naturally generate without
...
accountancy --- the opposite of the human capital prediction that
...
directly measurable. \citet{MockettiRomaRubolino2022} exploit two
waves of liberalisation of professional services in Italy and find
that deregulation reduces intergenerational occupational
persistence among less able dynastic workers first --- a pattern
better explained as a regime shift in what the competition rewards
than as uniform skill transfer.\footnote{A related finding concerns
...
\citet{BeckerKominersMurphySpenkuch2018} acknowledge in the most
recent extension of the Becker-Tomes framework that prior human
capital theories ``offer no clear explanation for the low mobility
in upper parts of the income distribution'' --- precisely the
regime in which the present model operates.
...
These results position the paper relative to its closest predecessors
as follows. The two closest
quantitative predecessors each reach conclusions that diverge from
the present paper by fixing the participant pool.
...
The closest predecessor that must be explicitly distinguished is
...
inequality. The present paper diverges on three dimensions that
are consequential for the results. First, the mechanism here is
a contest rather than a signaling game: social capital enters
the composite score directly as a productive rank-building input,
with no asymmetric information about pre-existing types and no
separating equilibrium. Second, \citeauthor{GalliceGrillo2019}
study the effect of social concerns on the \emph{level} of
educational investment conditional on participation; the present
paper takes a step back and asks which agents participate at all,
making participation itself the endogenous object. Third, the
threshold participation cost --- absent from
\citeauthor{GalliceGrillo2019} --- generates the binary
equilibrium outcomes, the closed-form participation threshold,
and the mobility ceiling that constitute the paper's main
contributions. Whether inequality amplifies or dampens status
investment conditional on universal participation is a distinct
question from whether inequality sustains or destroys
participation itself.

A broader literature on social capital and credential investment
motivates the threshold cost structure
\citep{Jencks1979, HeckmanMosso2014}. Participation costs in elite
competitions --- tuition fees, membership dues, network cultivation
...
MijsParadox2019, MijsMeritocracy2020} motivates the empirical
puzzle and the Case~II over-investment result, but the model does
...
the existence of the binary equilibrium structure (Cases~I and~IV
...
it appears in both Case~I and Case~II:
...
$Q=1$ the Case~II window is wide ($\Phi(1)\approx0.46$ at $\mu=0.2$),
...
Proposition~\ref{prop:caseII} --- that Case~II requires
...
(Proposition~\ref{prop:asymptotic_phi}), making Case~II
quantitatively relevant only for small competitions ($Q=1$ or $2$).
This is consistent with the interpretation that Case~II ---
...
participant's Case~IV incentive condition $U_{2}<Q\cdot\Phi(Q)$
weakens as $Q\cdot\Phi(Q)\to0$
(Corollary~\ref{cor:caseII_impossible}). The Case~IV ceiling ($\to1$) is relevant only when $Q$ is
...
(Case~IV) and the elite-only equilibrium (Case~I) is governed jointly
...
(Case~I), collective low-income mobility --- the probability that
...
all admissible $\mu < \tfrac{1}{2}$, and is increasing in $\mu$.
...
low-income-origin participant, as $Q\to\infty$ under Case~I.}
\end{minipage}
\end{table}
...
--- including \citet{ChettyLandofOpportunity2014} --- consistently
report that children born into the bottom income quintile have
roughly a 10--20\% probability of reaching the top quintile.
These figures are consistent with the model's ceiling under
$\mu \in [0.20, 0.30]$, a range that corresponds to competitions
...
empirically relevant regime. Under universal participation (Case~IV),
...
competitions. It is the Case~I ceiling that constitutes the
...
Cost reductions that shift the equilibrium from Case~I to Case~IV
...
The model's comparative statics on $Q$ and $\mu$ generate a sharp
...
window $A$ is relatively wide, and both Cases~I and~IV are plausible
...
Case~II equilibrium --- in which low-income participants over-invest
...
Policy targeting small competitions should therefore combine cost
reduction with efforts to lower $\mu$: introducing structured
competency-based assessment criteria, reducing the weight placed on
network affiliation in hiring decisions, and increasing transparency
about what assessors actually value. Without the latter, cost
reduction alone may merely extend the over-investment trap to a
larger pool of low-income applicants.
...
be Case~I regardless of cost levels, this is the operative lever.
...
to shift the equilibrium from Case~I to Case~IV, the ceiling on
collective mobility jumps from $\mu/[2(1-\mu)]$ to the Case~IV
...
within the Case~I regime without improving mobility outcomes. The
...
leads to policies that improve one dimension without improving
the other.
...
elite credentials in the Weberian-credentialist tradition: social
...
generically stable: universal participation (Case~IV) and
elite-only participation (Case~I). The transition is governed by
...
Third, a reference-dependent equilibrium (Case~II) exists in which
...
essentially unchanged \citep{LiEtAl2017}. The mobility ceiling
is consistent with Russell Group access rates of 1--1.4\% against
a theoretical equality benchmark of 4.4\% --- a directional and
structural correspondence, not a direct numerical mapping. The
PIAAC decomposition of the Great Gatsby Curve \citep{GGCPiaac2015}
is consistent with the participation threshold as a distinct
mobility-reducing channel operating on access rather than returns.
The three empirical residuals introduced in the introduction ---
...
as set out there. In each case human capital theory predicts the
wrong sign or cross-sectional pattern; the contest model predicts
the correct one.
...
This complementarity implies a specific diagnosis of why
two decades of widening participation policy in the United Kingdom
--- through Office for Students access targets, contextual
admissions, and bursary programmes --- have produced only modest
...
reductions required to move from Case~I to Case~IV. Simultaneously,
...
The policy prescriptions differ sharply between small and large
competitions. In small competitions --- partner-track interviews,
senior academic appointments, narrow graduate schemes --- both
instruments have traction, and the Case~II over-investment
pattern means that cost-reduction policy alone may extend
credential over-investment to a wider pool without raising
mobility. The appropriate intervention here combines cost
reduction with active reduction of $\mu$: structured
competency-based criteria, reduced weight on network affiliation,
and transparent assessment rubrics. In large competitions ---
...
equilibrium-shifting effect, not on signaling arguments.
...
generating social returns --- are applying the wrong theoretical
framework to positional competitions where the investment
directly raises competitive rank. In positional contests, the
...
The participation threshold prediction is similarly testable:
the model implies that the marginal effect of a credential
subsidy on participation rates should be non-linear, with small
reductions below the threshold producing no participation
response and reductions that cross the threshold producing
discrete jumps. The Chinese and UK widening participation
evidence already suggests this non-linearity; a direct test
using variation in subsidy generosity around the threshold
would identify the equilibrium-shifting effect that the model
isolates as the welfare-relevant margin.

\medskip
\noindent\textbf{Scope and extensions.} Several assumptions
bound the analysis and point to natural extensions. The model
restricts attention to a single high-income participant;
relaxing this to multiple incumbents preserves the binary
equilibrium structure but modifies the ceiling formula, and
is left to future work. The threshold cost $\overline{\nu}$
is treated as a recurrent period cost rather than a lumpy
one-time investment; a savings margin allowing low-income
participants to accumulate toward entry over multiple periods
would widen the feasibility window for Case~IV. The income
positions $y_1$ and $y_2$ are exogenous: the model identifies
how inequality shapes participation given a distribution of
positions, but not how status competition feeds back into the
income distribution over time. Integrating the participation
mechanism with an endogenous income process --- closer in
spirit to \citet{ColeMailaithPostlewaite1992} --- would allow
the model to speak to the long-run co-evolution of inequality
and credential investment that the empirical literature
increasingly documents \citep{MijsMeritocracy2020, ChettyLandofOpportunity2014}.
...
$Q\cdot\Phi(Q) \to 0$ at rate $1/Q$. The Case~II feasibility
condition $U_1 < \Phi(Q)$ fails for all $U_1 > 0$ for sufficiently
large $Q$: Case~II is asymptotically impossible.
\end{corollary}
