...
The decision to invest in elite professional credentials --- an MBA,
a CFA qualification, membership of exclusive networks --- is rarely
...
raw talent under Assumption~\ref{assu:mu_less_than_half}. Following
...
This binary structure formalises the intuition behind Hirsch's
(\citeyear{HirschSocialLimits}) social limits to growth and provides
...
rising status expenditure can coexist --- a result the post-war
literature could not generate. Third, we characterise precisely when the standard equilibrium
...
which equilibrium prevails: $\mu/[2(1-\mu)]$ under elite-only
participation and $1$ under universal participation. The former is
the binding ceiling: the comparative statics on $Q$ drive the
...
The paper proceeds as follows. Section~\ref{sec:Literature-Survey-1}
...
ceilings. Section~\ref{sec:Conclusion} concludes.
...
which participation is an endogenous equilibrium outcome. The key
architectural choice --- fixing one high-income participant and varying the
pool of low-income challengers $Q$ --- is not a generalisation of any prior
published two-player model but a deliberate modelling decision that
makes the participation question well-posed: when $Q$ is a free parameter,
participation thresholds, competition-size comparative statics, and the
divergence between individual and collective mobility all become
derivable objects.

The three closest quantitative predecessors each address the fundamental
question of how inequality shapes status competition, but their answers
diverge from the present paper for a structural reason that runs deeper
than modelling convention.

\citeA{FrankAER1984} establishes that status is a reciprocal phenomenon:
one worker's gain in rank comes only at the expense of another's loss,
and when workers are free to choose their coworkers, competitive wage
structures must be ones in which wage differences understate marginal
product differences. Workers accept wage penalties to occupy the top
of the local rank distribution within a firm. The inequality implication
is unambiguous: wider wage dispersion within a firm *reduces* competition
intensity, because the cost of low status falls as the gap between top
and bottom widens. Narrowing inequality, by raising the cost of low
status, *intensifies* positional rivalry. Frank's model therefore predicts
that inequality and status competition move in opposite directions ---
precisely the post-war prediction contradicted by empirical evidence.
Since all workers are employed somewhere and the only decision is which
firm to join, there is no participation threshold and no income constraint
on entry: the participation margin that drives our results does not exist
in Frank's framework.

\citeA{ColeMailaithPostlewaite1992} derive status concern endogenously
from the marriage market rather than placing it directly in the utility
function --- a methodological choice our paper adopts. In their model,
agents save beyond what consumption alone would justify in order to
improve their children's matching prospects; this competitive over-saving
is a prisoners' dilemma in which all agents participate by assumption.
Their own comparative static on inequality makes the directional
prediction explicit: an agent from an economy with a more compact income
distribution faces a higher marginal return to bequest ($m'_{t+1}$
is larger when the capital distribution is tighter), and therefore saves
more. They state directly that two men with the same initial income
level will exhibit higher savings rates in the economy with the more
compact income distribution, all else equal \cite[p.~1103]{ColeMailaithPostlewaite1992}.
The prediction is the same as Frank's: narrowing inequality intensifies
status competition. The reason is also the same: the behavioural margin
is *intensity of action within a fixed participant pool*, not whether to
participate at all. There is no threshold cost and no income constraint
on marriage market entry; the question of whether low-income agents can afford
to compete is never asked.

\citeA{moldovanuselacontestsstatus2007} introduce agent heterogeneity
and endogenous effort into a status contest, but the participation
structure is the reverse of ours: their Theorem~2 establishes that a
designer who optimally adjusts the partition of status categories in
response to additional entry can always ensure that total effort
increases. The designer therefore has no incentive to restrict entry,
and all agents always compete. Their individual rationality constraint
in the monetary prizes extension (Section~4) sets the minimum prize
$V_1 = n - r_1$, ensuring the lowest-type contestant is just willing
to participate --- but this is a mechanism design instrument, not an
income constraint. The designer's budget $P$ is always available to
compensate any participant; the possibility that a low-income agent's income
$y_1$ is insufficient to cover a fixed entry cost $\overline{\nu}$
is structurally absent. Their inequality result --- that a more
concentrated ability distribution justifies coarser prizes with a
unique top winner --- is a result about prize architecture given full
participation, not about who can afford to enter.

The common thread across all three predecessors is that inequality
affects the *intensity of behaviour within a competition* but cannot
affect *who enters*. Frank's workers always join some firm;
Cole, Mailath and Postlewaite's agents are always in the marriage market;
Moldovanu, Sela and Shi's agents always compete, and their designer
actively ensures this. The present paper introduces the one change that
breaks this pattern: a fixed threshold cost $\overline{\nu}$ that is
feasible for the high-income agent by construction and feasible for the low-income agent
agent only when $\overline{\nu} < y_1$. Once participation is
income-constrained, wider inequality simultaneously sustains high-income
investment (Case~I incentive rises) and prices out low-income participants (threshold
looms larger relative to $y_1$) --- generating the observed positive
association between inequality and status expenditure that no predecessor
can produce. The participation threshold $\overline{\nu}^{*}$, the
binary equilibrium structure, the comparative statics on $Q$ and $\mu$,
and the mobility ceiling are all direct consequences of this single
...
and inequality can rise together --- a result the post-war literature
could not generate. Third, reference-dependent utility introduces a
