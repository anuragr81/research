...
Player $i$ spends $m_i \geq 0$ on marketing. This constitutes soft
...
Each player allocates budget $W_i > 0$ across: marketing spend $m_i$,
own-process investment $(1-\alpha_i)W_i^{\mathrm{eff}}$, and
cross-investment $\alpha_i W_i^{\mathrm{eff}}$ in the other's process,
where $W_i^{\mathrm{eff}} = W_i - m_i$ and $\alpha_i \in [0,1]$.
Incoming cross-investment multiplicatively augments the receiving
player's capital base (it is genuinely deployed into the process). A
performance fee $f \in (0,1)$ is retained on the gross return earned by
the other's capital. Terminal wealth:
\begin{equation}
  W_i(T) = r_i^t\underbrace{(W_i^{\mathrm{eff}}
    + f\alpha_j W_j^{\mathrm{eff}})}_{\tilde{A}_i}
...
$\rho_i > 0$, using true $(\mu_i, \sigma_i^2)$ for their own process
and Jeffrey-shifted beliefs $(\mu_j^i, \sigma_j^{i2})$ for
the other's process.
