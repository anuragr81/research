...
The discrimination rate $\phi^t = F(\bar{c}_g^t)$ is determined by the
engagement threshold (\ref{eq:threshold}). The composed map is:
\begin{equation}
  H(\bar{c}_g) = \mu + \bar{c}_g(1 - \mu + \tilde{\gamma} - \delta F(\bar{c}_g))
...
equilibrium $\bar{c}_g^L(g_2)$ independently of $g_1$'s trajectory, and
