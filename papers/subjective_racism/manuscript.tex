...
The model uses a the framework of probability
kinematics developed by \citet{Jeffrey1965} to connect the three observations. More particularly, an agent revises their probability distribution over a partition
$\{g_1, \ldots, g_K\}$ (group memberships) without receiving a
definitive signal about which partition element is true. The conditional probabilities
...
stereotype thus persists. 

The paper makes third contributions. First, we prove that the Jeffrey
...
counter-examples by construction. If we can assume that economic class generates social clustering, then a structurally
stable trap follows. Third, we derive a threshold level of intergenerational
...
(equation~\ref{eq:welfare}, Table~\ref{tab:welfare}). 
