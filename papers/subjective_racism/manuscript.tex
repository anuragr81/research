...
Post-colonial literature has repeatedly identified a structural
feature of discrimination that economics has not formalised:
individual assimilation is individually rewarded and collectively
futile. We provide the mechanism. When the operative signals in
social interaction are soft---accents, dress, consumption patterns,
phenotype---the theoretically appropriate updating rule is Jeffrey
conditioning, which revises beliefs about group membership while
holding group-level class distributions fixed. The consequence is
a rigidity condition under which individual counter-examples are
absorbed without revising the group prior: the mimic man is an
equilibrium outcome, not a literary conceit.
A marker salience parameter governs the strength of the Jeffrey
revision, distinguishing mutable markers (accent, dress) from
immutable ones (phenotype, skin colour); together with the group
prior, these two sufficient statistics---one spatial, one
temporal---exhaust the observational inputs to the discrimination
mechanism and explain why phenotypic proximity to the host group
dominates conflict history in determining integration speed.
The model generates endogenous closure equilibria that are
path-dependent on economic history and marker salience, and
sustained by platform agents that produce informationally laundered
admission policies. We show that class-based social closure exists
in a racially homogeneous society as a structural feature of the
model, that racial discrimination is a perturbation of this
underlying mechanism, and that there exists a threshold level of
...
eliminated through a saddle-node bifurcation. The platform
...
closure equilibrium, marker salience, intergenerational mobility,
platform economics, mimic men.
\end{minipage}
\end{center}
...
Three observations motivate this paper---but they are preceded by a
recognition. The post-colonial literary tradition has repeatedly
identified a structural feature of discrimination that economics
has not formalised. Naipaul's Ralph Singh, in \emph{The Mimic Men}
(1967), invests everything in the markers of the colonial metropole
and achieves personal admission to its institutions. He is
reclassified. His group is not. Fanon's colonised subject discovers
that the white mask does not revise the white gaze. Coetzee's
post-apartheid figures navigate a world whose legal structure has
changed overnight while its inferential architecture has not. The
dilemma these writers identify is precise: individual assimilation
is individually rewarded and collectively futile. The exception is
absorbed without revising the rule. This paper provides the formal
mechanism. When the operative signals are soft and the theoretically
appropriate updating rule is Jeffrey conditioning rather than
standard Bayes, the compliance route fails by construction. The rule
does not revise. The mimic man is not a literary conceit but an
equilibrium outcome.
...
distinguishable from existing discrimination models. Five are
...
\textbf{Prediction 4: Marker salience dominates conflict history.}
Corollary~\ref{cor:salience-threshold} predicts that integration
speed depends more strongly on phenotypic distance from the host
group than on the severity of historical conflict. Groups with
low $s_g$ and high conflict history should integrate faster than
groups with high $s_g$ and low conflict history. This is directly
testable by comparing integration trajectories---intermarriage
rates, residential segregation indices, occupational
convergence---across immigrant groups in the same host country,
controlling for initial economic position and holding the
institutional environment fixed.

\textbf{Prediction 5: UK--US composition of discrimination.}
The space-time sufficiency structure
(Remark~\ref{rem:spacetime}) generates a cross-country prediction.
The UK has a deep pre-existing class closure trap with a rich
temporal marker space: accent, school, dress, and linguistic
register encode class history with high precision, so the temporal
dimension $\bar{c}_g$ does more of the discriminatory work. A
high-class foreigner whose temporal markers read correctly within
the British system receives a high initial prior regardless of
race---race is a second-order modifier. The US has a shallower
class marker space: fewer independent temporal markers are available
to override the spatial (racial) signal, so the spatial dimension
$s_g$ carries more of the informational load. The prediction is
that a high-class foreigner from a racially stigmatised group
should experience more racial friction in the US than in the UK,
but more class friction in the UK than in the US. Total
discrimination may be similar in magnitude but differs in
composition. The shadow information market
(Proposition~\ref{prop:shadow}) is organised around temporal
signals in the UK (the ``old boy network'') and spatial signals
in the US.
