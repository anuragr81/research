...
$\pi_0(c \mid g_k)$---the group-level priors over economic class---are
...
\emph{not} update $\pi_0(c \mid g_k)$ for the group as a whole. The
...
holding the conditional distributions $\pi_0(c \mid g_k)$ fixed ---
...
The model takes the initial group-level priors $\pi_0(c_H \mid g_k)$
...
threshold $\pi_0^{\text{excl}}$ (Proposition~\ref{prop:exclusion}):
...
tipping point $\pi_0^{\text{mid}}$. This simultaneously vindicates
...
$d(c_i, c_j) \leq \bar{d}$, where $d: \C \times \C \to \R_+$ is
a distance function and $\bar{d}$ is the compatibility threshold.
...
\begin{equation}
  \pi_0(c_H \mid g) = \frac{1}{T} \sum_{t=1}^{T}
  \mathbf{1}[c_{g,t} \geq \bar{c}]
  \label{eq:prior}
\end{equation}
where $\bar{c}$ is the threshold above which class is ``high'' and
$T$ is the historical window. The prior is a \textbf{sufficient
statistic of the group's economic history}. Groups with histories of
marginalisation---slavery, colonial extraction, persistent
poverty---have low $\pi_0(c_H \mid g)$.

\begin{remark}[Bernoulli structure]
\label{rem:bernoulli}
If individual class outcomes within a group are modelled as
exchangeable Bernoulli trials with success probability $\pi_0$, then
$\pi_0$ is the unique sufficient statistic by the De~Finetti
representation theorem \citep{DeFinetti1964}, and the within-group
variance of class outcomes is $\pi_0(1-\pi_0)$. This variance plays
...
\begin{equation}
  P'(c) = \sum_{k=1}^{K} \pi_0(c \mid g_k) \cdot P'(g_k)
  \label{eq:jeffrey}
\end{equation}
...
$\pi_0(c \mid g_k)$ are held fixed during the Jeffrey revision. Only
...
does \emph{not} update $\pi_0(c \mid g)$. This is the formal
...
Agent $j$ engages iff expected class distance does not exceed the
...
$\partial \E[d]/\partial c_j = 1 - 2\pi_0 < 0$ when $\pi_0 > 1/2$.
\end{proposition}
...
reducing weight on group prior $\pi_0(c \mid g)$.
\end{proposition}

\begin{proposition}[Economic history effect]
\label{prop:history}
Lower $\pi_0(c_H \mid g)$ generates higher posterior expected
incompatibility for any given marker, pushing more potential partners
below the engagement threshold:
$\partial \E[d]/\partial \pi_0 = c_H + c_L - 2c_j < 0$ when
$c_j > (c_H + c_L)/2$.
\end{proposition}
...
strong one. $\E[d]$ is linear in $\pi_0$
($\partial^2 \E[d]/\partial \pi_0^2 = 0$), distinguishing the model
...
\begin{equation}
  \pi_0^{t+1}(c_H \mid g) =
  \underbrace{\pi_0^t(1 - \delta \phi^t)}_{\text{exclusion erodes prior}}
  + \underbrace{\mu(1 - \pi_0^t)}_{\text{natural convergence}}
  + \underbrace{\gamma \cdot \pi_0^t(1-\pi_0^t)}_{\text{Bernoulli mobility}}
  \label{eq:dynamic}
\end{equation}
...
The Bernoulli mobility term $\gamma \cdot \pi_0^t(1-\pi_0^t)$ is
the within-group variance of class outcomes under the exchangeable
Bernoulli model (Remark~\ref{rem:bernoulli}). It is maximised at
$\pi_0 = 1/2$ (maximum heterogeneity) and vanishes at $\pi_0 = 0$
and $\pi_0 = 1$ (homogeneous group). This term captures
...
The discrimination rate $\phi^t = F(\pi_0^t)$ is determined by the
engagement threshold (\ref{eq:threshold}). The composed map is:
\begin{equation}
  H(\pi_0) = \mu + \pi_0(1 - \mu + \gamma - \delta F(\pi_0))
  - \gamma \pi_0^2
  \label{eq:composed}
\end{equation}

The dynamic system connects the interaction decision of Section 3.4
to the group prior through three channels that operate simultaneously.
...
which discrimination erodes the group prior across generations.

\begin{definition}[Closure Equilibrium]
\label{def:closure}
A closure equilibrium is a fixed point $(\phi^*, \pi_0^*)$ of the
...
$\pi_0^L < \pi_0^H = 1$, separated by an unstable tipping point
$\pi_0^{\mathrm{mid}}$. The low fixed point constitutes a
...
Define $\Psi(\pi_0) = H(\pi_0) - \pi_0$. Then $\Psi(0) = \mu > 0$
...
follows from $|H'(\pi_0^L)| < 1$ and $|H'(\pi_0^{\mathrm{mid}})| > 1$.
Path dependence follows from the monotonicity of orbits on each side
of $\pi_0^{\mathrm{mid}}$. \qed
\end{proof}
...
\begin{enumerate}[noitemsep]
  ...
  (unique stable fixed point at $\pi_0 = 1$).
...
$\pi_0^0(c_H \mid g_2) < \pi_0^{\mathrm{mid}}$---their prior starts
in the high-discrimination basin.
\end{definition}

\begin{proposition}[Enemy group trap]
\label{prop:enemy}
If $g_2$ enters with $\pi_0^0(c_H \mid g_2) < \pi_0^{\mathrm{mid}}$
...
equilibrium $\pi_0^L(g_2)$ independently of $g_1$'s trajectory, and
$g_1$'s equilibrium is unaffected.
\end{proposition}
...
\begin{enumerate}[noitemsep]
  \item $\Omega_j$ is decreasing in $\pi_0$ for $\pi_0 < 1/2$:
...
At the enemy group's entry prior $\pi_0^0(g_2)$, the platform
...
\label{prop:tokenism}
The platform exclusion threshold $\pi_0^{\mathrm{excl}}$ satisfies
$\pi_0^{\mathrm{excl}} < \pi_0^{\mathrm{mid}}$: the platform begins
admitting $g_2$ members \emph{before} they have escaped the
high-discrimination basin, creating a partial inclusion zone
$[\pi_0^{\mathrm{excl}}, \pi_0^{\mathrm{mid}}]$ in which $g_2$ has
...
If $g_1$'s equilibrium prior $\pi_0^*(g_1)$ is close to
$\pi_0^{\mathrm{mid}}$, the platform's admission of $g_2$ members
at $\pi_0^{\mathrm{excl}}$ can push $g_1$ below its own tipping
...
fail to update $\pi_0(c \mid g)$; the belief system never corrects
...
rigidity parameter $\rho > \rho^*$ and the initial group prior
$\pi_0$.
\end{corollary}
...
$\pi_0(1-\rho)(c_H - c_L)$---the component of belief error that
...
conditional probabilities $\pi_0(c \mid g)$. The gap is therefore
...
$\pi_0^{\text{mid}}$) but does not alter the trap's structure.
...
in observers' Jeffrey revisions toward a low-$\pi_0$ group prior.
...
updating $\pi_0(c \mid g)$. Gender norms are therefore sticky even
...
name is received by a person whose appearance suggests a low-$\pi_0$
...
$\pi_0(c_H \mid g)$ for the group as a whole. The next cross-group
...
$P'(g_k)$ without updating $\pi_0(c \mid g)$ for the group.

The compounding effect arises because the two revisions interact.
Let $\pi_0^A$ denote the prior over the accused's character
(probability of innocence, say) and $\pi_0^C$ denote the prior over
...
$\pi_0^A$ is anchored by the accused's group markers,
$\pi_0^C$ is anchored by the accuser's group markers, and the joint
...
race-correlated low $\pi_0^A$ and the accuser carries a
gender-and-race-correlated high $\pi_0^C$, the compounded
...
other: a downward-biased $\pi_0^A$ and an upward-biased $\pi_0^C$
...
accuser is a white woman. The observer's $\pi_0^A$ is shaped by
...
inference architecture. The observer's $\pi_0^C$ is shaped by the
...
not update $\pi_0^A$ for the group. The next accused Black man is
assessed from the same group prior. Similarly, a credibility
assessment that finds the accuser unreliable shifts $P'(g_k)$ for
this particular accuser but does not update $\pi_0^C$ for white
...
her race-correlated $\pi_0^C$ is lower than a white woman's in the
same institutional context, while her gender-correlated $\pi_0^C$
...
$\pi_0^A$ through racial equity programmes and reducing $\pi_0^C$
...
$\pi_0(c_H \mid g)$ for the group. The next passenger is assessed
...
individual compliance does update $\pi_0(c_H \mid g)$ for the group:
...
toward the truth. Enough counter-examples, and $\pi_0$ crosses the
tipping point $\pi_0^{\mathrm{mid}}$; the closure trap dissolves.
...
revision. The group prior $\pi_0(c_H \mid g)$ remains in the
...
compliance can drive the group prior across $\pi_0^{\mathrm{mid}}$,
...
$\pi_0^{\mathrm{mid}}$ through the mobility mechanism
...
of immigrants and minorities (proxies for $\pi_0$), own income,
...
\textit{Prior measurement}: $\pi_0(c_H \mid g)$ is latent. Use
...
equilibrium --- a second group whose prior $\pi_0(c_H \mid g_2)$
...
update the group prior $\pi_0(c \mid g)$---standard Bayesian updating.
At $\rho = 1$, the group prior is completely rigid---full Jeffrey
conditioning. The amended dynamic system is:
\begin{equation}
  G_\rho(\pi_0, \phi) = \mu + \pi_0(1 - \mu - \rho \delta \phi)
  + \gamma \pi_0(1-\pi_0)
  \label{eq:loury-rho}
\end{equation}
...
\begin{enumerate}[noitemsep]
  ...
  point at $\pi_0 = 1$. Individual counter-examples erode
...
toward high-class signals, effectively raising the $\pi_0$ perceived
by others. The marginal benefit of investment is:
\begin{equation}
  \mathrm{MB}(e) = -(c_H + c_L - 2c_j) \cdot \theta \cdot
  (v - \bar{u}) \cdot (1 + \xi \phi^*)
  \label{eq:MB}
\end{equation}
where $\theta > 0$ is marker sensitivity and $\xi > 0$ captures the
amplification from trap severity $\phi^* = F(\pi_0^L)$.

\begin{proposition}[Endogenous marker investment]
\label{prop:veblen}
The equilibrium marker investment is:
\begin{equation}
  e^* = \frac{(2c_j - c_H - c_L) \cdot \theta(v-\bar{u})(1+\xi\phi^*)}
  {\kappa_e}
  \label{eq:estar}
\end{equation}
with the following properties:
\begin{enumerate}[noitemsep]
  \item $e^* > 0$ iff $c_j > (c_H + c_L)/2$: only agents above
  ...
  with $c_j < (c_H + c_L)/2$, producing a separating equilibrium:
...
escape the closure trap from initial position $\pi_0^0$ scales as:
\begin{equation}
...
\pi_0^2$ evaluated at the bifurcation point.
\end{corollary}
...
$\Psi \approx a(\gamma - \gamma^*) + b(\pi_0 - \pi_0^*)^2$ near the
