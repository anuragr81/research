...
pushes $\gamma$ toward $\gamma^*$. Platform regulation reduces
...
The examples below illustrate four faces of this interaction:
incentives that produce conflict (\S\ref{subsec:cluster-produce}),
conflict that reproduces the economic structure
(\S\ref{subsec:cluster-reproduce}), strategic action over the
parameters themselves (\S\ref{subsec:cluster-parameters}), and
boundary cases that delimit when the mechanism applies
(\S\ref{subsec:cluster-boundary}).
...
rank. What the model does claim, in the tradition of Popperian
piecemeal analysis, is that well-intentioned formal interventions
...
by economic actors pursuing private returns. Restaurants, fashion
and beauty markets, urban developers, and institutional gatekeepers
do not encounter a pre-existing social order; they construct one.
The conflict they generate is not a market failure but an
equilibrium outcome of their incentives.

\subsubsection{The restaurant: platform-mediated belief architecture}
...
\subsubsection{Fashion and beauty: Veblen dynamics and the foreign influence}

Fashion and beauty standards perform the function
\citet{Goffman1959} identifies as the social staging of the self:
they allow individuals to claim membership in an economic class
through visible signals. The endogenous marker investment result
(Appendix~\ref{app:veblen}) formalises why: the return to investing
in class-signalling markers is increasing in the severity of the
closure trap, because the trap raises the cost of being
misclassified as low-class.

The relationship of beauty standards to foreign groups follows the
same logic. If a conflict or historical episode associates a foreign
group with economic defeat or contamination, appearing as someone
from that group becomes an informationally costly marker---it shifts
$P'(g_k)$ in observers' Jeffrey revisions toward a low-$\bar{c}_g$
prior. Symbols of foreign cultures that encode dominance over the
host---the Veblen trophy dynamic---raise the value of those symbols
as status markers. Whether a foreign influence is incorporated or
rejected by beauty and fashion industries is thus a function of the
economic implications it carries for those who signal with it, not
of intrinsic aesthetic merit. The model provides a unified account
of both the attraction and the repulsion.

\subsubsection{Credential checks and protective intervention}
...
\subsubsection{Gentrification as platform optimisation}
...
the sudden-legal-change result (\S\ref{subsec:sudden-law}) applies:
$\sigma$ is constrained, the shadow market intensifies, and
discrimination redirects rather than disappears.
...
rigidities already in place---gender norms, racial stereotypes,
institutional credibility hierarchies, historical adverse selection
patterns---propagate themselves across generations and across cases,
sustaining the economic structure that produced them. The Jeffrey
architecture is the mechanism by which conflict reproduces the
incentives that produced it.

\subsubsection{Gender norms: inferential persistence regardless of origin}
...
only that, whatever their source, the inferential mechanism that
sustains class closure sustains gender closure by the same formal
route.
...
updating $F_g$. Gender norms are therefore sticky even when
individual deviations are common and widely tolerated, because the
social inference process cannot propagate individual deviations into
group prior revision. The positive-analytical claim is limited to
this: the Jeffrey updating rule sustains gender norms against
individual counter-examples by the same structural mechanism that
sustains racial and class stereotypes, and formal policies that
...
\subsubsection{Compounding markers: race, gender, and institutional
...
assessments is a case in point.
...
revisions: one over the accused's character using markers correlated
with the accused's group, and one over the accuser's credibility
using markers correlated with the accuser's group and social
position. Both revisions are governed by the rigidity condition.
Let $\bar{c}_g^A$ denote the prior over the accused's character and
$\bar{c}_g^C$ denote the prior over the accuser's credibility. The
joint posterior is the product of two independently rigid revisions,
and the compounding follows directly. If the accused carries a
race-correlated low $\bar{c}_g^A$ and the accuser carries a
gender-and-race-correlated high $\bar{c}_g^C$, the compounded
distortion exceeds what either marker dimension would produce alone.
The two rigidities reinforce each other: a downward-biased
$\bar{c}_g^A$ and an upward-biased $\bar{c}_g^C$ jointly push the
institutional assessment toward crediting the accusation
independently of its evidentiary content.

The case that makes this concrete is a harassment accusation in
which the accused is a Black man and the accuser is a white woman.
The observer's $\bar{c}_g^A$ is shaped by the historical prior over
Black male criminality---itself a sufficient statistic of the group's
racialised economic and legal history. The observer's $\bar{c}_g^C$
is shaped by the historical construction of white female testimony
as inherently credible in this institutional context, a prior that
also reflects economic and social history rather than case-specific
evidence. Under bilateral Jeffrey conditioning, both priors are rigid
to the specific facts of the case. The joint posterior is
systematically biased in the direction of crediting the accusation
even when the evidentiary record is ambiguous---not because any
individual actor is malicious, but because the inference architecture
compounds two independently rigid priors in a mutually reinforcing
direction. A case that ends in exoneration shifts $P'(g_k)$ for this
particular accused but does not update $\bar{c}_g^A$ for the group;
the next accused Black man is assessed from the same prior.
...
specific combination of group priors in play. A Black woman reporting
harassment by a white man faces a different compounding: her
race-correlated $\bar{c}_g^C$ is lower than a white woman's in the
same institutional context, while her gender-correlated $\bar{c}_g^C$
pushes in the opposite direction. The net effect is not the sum of
the racial and gender components but a non-additive interaction
between them---precisely the pattern that the sociological literature
on intersectionality identifies empirically \citep{Crenshaw1991},
here given a formal microfoundation in the Jeffrey architecture. The
welfare implication is that compounding discrimination is irreducible
to its component dimensions: reducing $\bar{c}_g^A$ through racial
equity programmes and reducing $\bar{c}_g^C$ bias through gender
equity programmes does not eliminate the compounding distortion if
each reform addresses only one dimension of a jointly rigid inference
architecture. This is a direct extension of the informational
irreducibility result (Corollary~\ref{cor:irreducibility}) to the
multi-marker setting.

\subsubsection{The Loury taxi driver revisited}
...
\subsubsection{Compliance, colonial promise, and the limits of
individual mobility}
\label{subsec:colonial}

Every historical closure system has offered a compliance route---the
promise that individuals who sufficiently adopt the host group's
markers will be reclassified and integrated. The \textit{\'{e}volu\'{e}}
in colonial Africa was the most formally codified instance: Africans
who attained a prescribed threshold of French or Belgian education
and cultural assimilation were granted a quasi-civil status. The
implicit contract was: invest in our markers, and we will revise your
group prior upward.
...
The third cluster covers cases in which actors attempt to change
the parameters of the inference architecture itself rather than
operate within it. Anti-discrimination law moves on $\sigma$;
...
parameters that can be changed quickly (law, formal admission policy)
and parameters that move only on intergenerational timescales (group
priors, marker salience, mobility rates).

\subsubsection{Sudden legal change and the space-time mismatch}
...
operates on the admission policy $\sigma$---a formal instrument that
is neither a spatial observation nor a temporal prior (see
footnote~\ref{rem:spacetime}). It changes overnight. The Jeffrey
prior ($\bar{c}_g$, $F_g$, $\rho$) and marker salience ($s_g$) are
untouched.

Three consequences follow immediately. The shadow information value
rises on impact: $\partial \Omega_j / \partial \lambda > 0$
(Proposition~\ref{prop:shadow}). Experience utility falls for the
host group, because social quality $S_j(\mathcal{A})$ depends on
Jeffrey-updated beliefs and $\bar{c}_g$ has not moved---the host
group's resistance has an economic-inferential foundation, not just
an ideological one. The result is the tokenism equilibrium of
Proposition~\ref{prop:tokenism}: visible inclusion without belief
revision. Without simultaneous investment in $\gamma$, sudden legal
change produces an outcome that erodes political support for both
instruments: the host group experiences disutility from forced
integration that it perceives as economically costly, the stigmatised
group experiences tokenism rather than genuine integration, and the
intensified shadow market provides the institutional infrastructure
for backlash. The historical pattern---post-Civil Rights Act sorting
in the American South, post-Race Relations Act class-mediated
exclusion in the UK---is consistent with the model's prediction.

\subsubsection{Language revival as voluntary \texorpdfstring{$s_g$}{sg} increase}
...
\subsubsection{Anti-colonial movements as collective action on
...
most important large-scale attempted solutions, and the model can
...
level. Pakistan retained British legal, military, and administrative
structures, English as the elite language, and cricket as the
national sport. Martinique remained a French d\'{e}partement.
Francophone West African states retained the CFA franc, French
military bases, and French as the official language. In model terms,
these countries changed the formal admission instrument but continued
investing collectively in $e^*$: metropolitan marker alignment. The
model predicts the tokenism equilibrium at the national scale.

\textbf{Antagonistic post-colonial relationships.} Other movements
explicitly rejected the compliance route. Nehru's import substitution,
promotion of Hindi, and non-alignment constituted a deliberate
attempt to build $\gamma$ independently of the metropole. Nyerere's
ujamaa was the most explicit: reject metropolitan markers entirely,
invest collectively in economic self-sufficiency. Algeria's FLN
ideology was anti-assimilation by design. The model identifies this
as the correct strategy for escaping the trap, but at a short-term
cost: the metropole withdraws the returns to compliance, and the
transition is painful precisely because the group is attempting to
cross $\gamma^*$ without the individual safety valve.

\textbf{Perpetual anti-colonial narrative.} Iran after 1979 rejected
the compliance route comprehensively, but $\gamma$ did not cross
$\gamma^*$ before the relationship was severed. The model predicts
that the anti-colonial narrative should persist precisely as long as
$\bar{c}_g < \bar{c}_g^{\mathrm{mid}}$: the narrative functions as a
coordination device preventing individually rational defection back
into compliance. If individuals defected into compliance, an adverse
selection dynamic would operate: the most capable individuals exit,
the remaining group's $\bar{c}_g$ falls, and the trap deepens. The
model predicts this narrative will dissolve endogenously if and when
$\bar{c}_g$ crosses the tipping point.
...
by the host country's observers can be contaminated by geopolitical
...
hostility. The model predicts that immigrants from politically
antagonistic states should face more discrimination than immigrants
from economically equivalent but politically neutral states, and
that this excess discrimination should track political relations
over time.

\subsection{Boundary cases probing the mechanism}
\label{subsec:cluster-boundary}

The final cluster collects three cases that probe the model's
applicability at unusual parameter configurations: zero marker
salience (digital anonymity), high marker salience combined with
high group economic position (the ``model minority''), and the
extension of the mechanism from persons to cultural objects.

\subsubsection{Digital de-anonymisation: a natural experiment on
\texorpdfstring{$s_g$}{sg}}
...
identity-revealing features: real names, profile photographs, video.
This produces a discontinuous jump in $s_g$ from near-zero to
whatever the natural marker environment supports. The model predicts
discrimination should emerge \emph{immediately} upon marker
revelation, with intensity proportional to the gap between the
...
population. The mechanism is pure spatial shock---the temporal
dimension is unchanged, but the spatial channel is switched on.

\subsubsection{The ``model minority'' configuration}
\label{subsec:modelminority}

East Asian immigrant communities in the US and UK occupy a region of
the $(s_g, \bar{c}_g)$ parameter space that the base model's worked
examples do not cover: high marker salience combined with high or
above-host group mean class ($\bar{c}_{g_2} \geq \bar{c}_{g_1}$).
Since $D_2 - D_1 = (\bar{c}_{g_2} - \bar{c}_{g_1})
(\bar{c}_{g_2} + \bar{c}_{g_1} - 2c_j)$ when variances are equal, the
sign depends on the observer's class relative to the midpoint. For
the median host-group observer ($c_j \approx \bar{c}_{g_1}$),
$\partial \E / \partial s_g > 0$---higher salience still increases
expected incompatibility, but the magnitude is small. For high-class
host observers ($c_j$ above the midpoint), the sign reverses: these
observers are closer to the out-group mean than to their own, and
higher salience makes the model minority \emph{more} attractive as
an engagement partner.

The result is a split pattern: economic integration with elite
gatekeepers combined with mild but persistent social friction from
the median host population. This is the ``bamboo ceiling'' pattern:
access to economic spaces where high-class gatekeepers select, but
exclusion from broader social spaces where median-class members
dominate. The model predicts this is a structural equilibrium, not
a transitional state, because the spatial markers are immutable and
the observer-position structure of $D_2 - D_1$ is fixed by the group
means.

\subsubsection{Cultural objects and the sign-value prior: a food
example}
\label{subsec:food}
...
same sign system grounded in economic
history.\footnote{This extension is compatible with the early-period
work of \citet{Baudrillard1968,Baudrillard1972} on sign value
grounded in economic history; the later hyperreality framework is
not invoked and would be in tension with the model's foundation in
$\bar{c}_g$ as a sufficient statistic of real economic history.}

A cultural object---hilsa curry, chicken feet, fermented soybean---carries
a low $\bar{c}_g$ in the host culture's prior that is rigid to
individual positive encounters by the same mechanism that sustains
person-level stereotypes. The bilateral updating structure of
Section~\ref{subsec:bilateral} applies with $c_j$ denoting the
object's sign position rather than a person's economic class. A
positive encounter shifts $P'(g_k)$ for \emph{this dish} without
updating $\bar{c}_g$ for the cuisine as a category.
...
is tighter for cultural objects than for persons. Cultural priors
are therefore more rigid than personal priors without any additional
assumption.
\end{remark}

Elites are more likely than the commons to cross the engagement
threshold for foreign cultural objects, through two mechanisms. The
first is the Veblen channel: elites face diminishing distinctiveness
returns on familiar markers, which endogenously lowers their
effective $\bar{u}$ for novel cultural objects and widens
$\Delta^*$. The second is the exposure channel:

\begin{quote}
\textbf{Assumption~E} (Exposure). Encounter frequency with foreign
cultural objects is increasing in own economic class:
$\lambda(c_i)$ is increasing in $c_i$. Empirical grounding: income
is positively correlated with international travel, specialist
restaurant patronage, and cultural consumption breadth.
\end{quote}
...
population, exposure frequency would equalise across class.
Individual reclassification would increase uniformly, but the group
prior $\bar{c}_g$ would remain rigid for the entire population. The
Jeffrey condition universalises individual reclassification without
driving the group prior across $\bar{c}_g^{\mathrm{mid}}$.
\end{remark}

Once elites cross the engagement threshold, the foreign cultural
object becomes a Veblen marker: its foreignness guarantees the
commons will not follow, and the marker's value to the elite is
precisely its inaccessibility to the class below. When the barrier
falls---the exotic food becomes mass-market---the commons'
$\Delta^*$ widens, the marker loses distinctiveness, and the elite
abandon it for the next barrier-protected object.\footnote{A full
dynamic treatment requires marker value at $t$ to decrease in the
fraction of the population that has adopted by $t$---the congestion
externality formalised by \citet{Leibenstein1950} and
\citet{CorneoJeanne1997}. The interaction with the Jeffrey
architecture is noted without further formalisation.}

The model generates a steady-state in which a positive fraction
$\phi$ of the population privately enjoy the foreign food while the
ambient group prior $\bar{c}_g$ for the cuisine remains low. The
Jeffrey rigidity condition establishes that $\bar{c}_g^t$ is
invariant to $\phi^t$. The steady-state has two independent fixed
points: one for $\phi$ (determined by exposure rate and income
distribution) and one for $\bar{c}_g$ (determined by the initial
prior and the rigidity parameter $\rho$). Crucially, $\phi$ can
approach 1 while $\bar{c}_g$ remains at its initial low value. This
explains a well-documented empirical pattern: foods widely enjoyed
by individuals from the host culture continue to be coded as
``ethnic'' or ``exotic'' in ambient cultural discourse long after
individual consumption has normalised. The food cycles through three
zones---disgusting, elite marker, mass-market---without the ambient
prior completing a full revision. This is a trap that operates on
cultural objects by the same mechanism that operates on group priors
for persons.
...
$\gamma^*$. Anti-discrimination law, applied
