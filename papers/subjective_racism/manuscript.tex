...
is reclassified, the group prior $\pi_0(c_H \mid g)$ does not move.
No volume of individual signalling can drive $\pi_0$ across the
mobility threshold $\pi_0^{\mathrm{mid}}$, because the Jeffrey
...
is the Beta distribution $\mathrm{Beta}(\pi_0 \nu,\,
(1 - \pi_0)\nu)$ with mean $\pi_0$ and variance
\begin{equation}
  \sigma_g^2 = \frac{\pi_0(1 - \pi_0)}{1 + \nu}
  \label{eq:beta-var}
\end{equation}
...
with variance $\pi_0(1-\pi_0)$, which is the parametrisation
...
high-$\pi_0$ groups---is assigned a high initial prior by the
...
associated with low-$\pi_0$ groups in the host's prior
...
bundle. The immigrant's initial prior assignment $\pi_0^{\text{init}}$
...
in $\G$), the observer defaults to a low-$\pi_0$ prior. Actual
...
revision to assign a high initial $\pi_0^{\text{init}}$ without
...
$\pi_0(c_H \mid g)$ for the cuisine as a category.
...
$\pi_0(c_H \mid g)$ would remain rigid for the entire population.
...
the ambient group prior $\pi_0(c_H \mid g)$ for the cuisine remains
low and codes it as disgusting. The Jeffrey rigidity condition
establishes that $\pi_0^t$ is invariant to $\phi^t$: the group prior
...
population income distribution) and one for $\pi_0$ (determined by
...
(everyone has privately enjoyed the food) while $\pi_0$ remains at
...
any conceivable aggregation mechanism could shift $\pi_0$. The food
