...
changes. At the population level, \citet{BisinVerdier2001} show that
cultural substitution sustains heterogeneous trait distributions
across groups through endogenous parental socialisation; at the
individual level, \citet{Bourdieu1984} documents that observable
markers---taste, dress, accent, bodily hexis---are durable
dispositions (habitus) resistant to economic mobility. The economic system determines class---income, wealth,
...
Section~\ref{sec:platform} develops the institutional channel: experience
utility, atmosphere vectors, shadow information markets, platform
exclusion, and tokenism. Section~\ref{sec:discussion} provides
worked examples including the restaurant case,
the colonial compliance mechanism, and strategic action over the
model's parameters.
...
\citet{BohrenImasRosenberg2019} show that Bayesian agents with
misspecified models of the data-generating process can sustain
persistent discrimination through biased learning; the departure in
their framework is the agent's subjective model, whereas in ours the
agent's model is correct and rigidity is a property of the evidence
itself.\footnote{A complementary
...
\citet{CoateLoury1993} identify both ``benign'' and ``patronising''
equilibria under affirmative action but do not derive a structural
condition that determines which outcome obtains. Recent work on the
contracting channel has made progress on this durability question.
\citet{ChaudhuriSethi2008} introduce peer effects into a
Coate--Loury-type framework and derive a threshold level of
segregation below which negative stereotypes become unsustainable.
\citet{BowlesLourySethi2014} show that segregation in social networks
combined with peer effects can sustain group inequality in skill
shares indefinitely even without labour market discrimination, and
identify a critical segregation threshold; they find that
redistributive policies can maintain group equality only so long as
they remain permanently in place. All of these results operate
through the contracting channel---labour markets, human capital
investment, employer screening. None address the contact channel of
informal social inference that the present paper formalises; our
bifurcation result (Proposition~\ref{prop:bifurcation}) provides a
threshold condition on intergenerational mobility for this channel.
...
experience \citep[population-level persistence:][]{BisinVerdier2001},
and they do not update when an individual's economic class changes
\citep[individual-level durability:][]{Bourdieu1984}. An agent who achieves $c_i = 1$ from a group with
...
% =============================================================================
\section{Institutional Aggregation: The Second Source of Persistence}
\label{sec:platform}
% =============================================================================
...
group prior $\bar{c}_g$ as the two governing parameters of
...
$\gamma^*$ eliminates the trap. The base model locates the first
source of persistence at the individual level: the rigidity condition
ensures that compliance does not aggregate into group prior revision.
This section identifies the second source, which operates at the
institutional level. Encounters have so far been treated as bilateral,
but in many real settings the environment in which they occur is
itself produced by a market actor: a restaurant calibrates its
atmosphere to attract preferred compositions, a school sets admission
criteria, a neighbourhood configures itself through pricing and
zoning. The two sources are independent: even if bilateral encounters
did not generate persistence, profit-maximising institutions acting
on soft signals would produce discriminatory compositions as an
emergent outcome.
...
bilateral model, with the institutional channel reinforcing it.
