...
membership while holding group-level economic class distributions
fixed.
...
reach of legal prohibitions on the contracting channel. Where the
formal channel is constrained, activity reorganises through a shadow
market of social information: coded language in reviews, word of
mouth in networks, selective social media.

We argue that the mechanism connecting these three observations is
\emph{Jeffrey conditioning}---the framework of probability
kinematics developed by \citet{Jeffrey1965}. The argument rests on a
structural distinction between two systems that interact but move at
different speeds. Following \citet{ColeMailathPostlewaite1992}, who
partition the economy into a market sector (savings, consumption,
capital accumulation) and a non-market sector (social status,
matching) governed by distinct allocation rules, we distinguish
between a \emph{social system} and an \emph{economic system}. The
social system produces markers---accents, dress, names, consumption
patterns---through cultural transmission, residential geography, and
historical experience. These markers are slow-moving: they are
properties of groups and communities, inherited rather than chosen,
and they do not update when an individual's economic position
changes. The economic system determines class---income, wealth,
occupation---which is mobile within a generation. An individual who
achieves upward economic mobility does not thereby shed the markers
of their origin group. This is the predicament of the ``mimic man''
of the title: the structural analogue of an agent in the aristocratic
equilibrium of \citet{ColeMailathPostlewaite1992}, whose inherited
social status persists irrespective of accumulated wealth.

Because markers are outputs of the social system rather than
diagnostic signals of current economic position, they carry no clean
within-group likelihood $P(m \mid c)$. The correlation between
marker and class is \emph{mediated by group}: a marker shifts
probability mass across groups, $P(g_k) \to P'(g_k)$, and the class
expectation follows from the group-level prior $F_{g_k}$---but the
marker provides no information that would revise $F_{g_k}$ itself.
This is precisely the evidential structure that Jeffrey conditioning
formalises: a direct reassignment of the partition weights without a
likelihood ratio, because no likelihood ratio exists to condition on
\citep{DiaconisZabell1982}. The conditional distributions $F_{g_k}$
are held fixed not by stipulation but because the soft signal, being
an output of the social system, provides no information capable of
revising them. Only the weights $P'(g_k)$ change. Jeffrey
conditioning is thus forced by the signal structure, not chosen as a
modelling convenience: where signals are hard enough to carry a
likelihood---the contracting channel of \citet{Loury2002}, studied by
\citet{Arrow1973} and \citet{Phelps1972}---standard Bayesian updating
applies and the prior does move.
...
under soft signals, not as an assumption about preferences or
cognitive bias.
...
\citep{Loury2002}, which Loury articulates conceptually but does not
formalise. The $\rho \to 0$ limit of the mechanism is the
standard-Bayesian benchmark of the statistical-discrimination
tradition \citep{Phelps1972,Arrow1973}; there, unbiased updating lets
counter-examples erode the stereotype, and contact-channel
discrimination arises only under Jeffrey rigidity, $\rho > \rho^*$
...
structural outcome (Proposition~\ref{prop:tokenism}), that
...
stigma as the mechanism sustaining discrimination in contact, but
does not formalise it. Existing formal models of self-confirming
stereotypes \citep{CoateLoury1993} derive persistent discrimination
from strategic belief--investment feedback under Bayesian employers;
we instead derive it from the structure of the updating rule, through
Jeffrey rigidity. Our model provides the
...
$\rho > \rho^*$; it is the formal analogue of the self-confirming,
persistent stereotype of \citet{CoateLoury1993}, generated here by
Jeffrey rigidity in the updating rule rather than by the
belief--investment feedback of their model.

Jeffrey conditioning is not merely a modelling choice. It is forced
by the two-system structure of the social environment the paper
studies. In the framework of \citet{ColeMailathPostlewaite1992}, an
economy is partitioned into a market sector---where capital, savings,
and consumption are determined---and a non-market sector---where
status, matching, and social allocation are governed by distinct
rules. The two sectors interact, but they need not move in step: in
their aristocratic equilibrium, inherited social status persists
regardless of accumulated wealth. We apply the same partition to the
informational environment. Markers (accents, dress, names) are
outputs of a slow-moving social system---cultural transmission,
residential geography, historical experience---while economic class
(income, wealth, occupation) is determined in a mobile economic
system. Because the marker is inherited from the social system rather
than generated as a diagnostic of current economic position, it
carries no likelihood function
$P(\text{marker} \mid \text{class})$. It shifts probability mass
across groups without resolving group membership: precisely the
evidential structure for which Jeffrey conditioning is the unique
updating rule that revises the marginal distribution over groups while
holding the conditional distributions $F_{g_k}$ fixed
\citep{DiaconisZabell1982}. Standard Bayesian updating requires a
likelihood function
$P(\text{signal} \mid \text{state})$, which presupposes that the
signal determines a definite evidential relationship with the state.
This presupposition holds on the contracting channel---credentials,
test scores, verifiable records---but fails on the contact channel,
where signals are soft. Alternative frameworks for updating under
...
cannot generate the closure trap as an equilibrium outcome. The
...
system: they are inherited at the group level through cultural
transmission, residential geography, and accumulated historical
experience, and they do not update when an individual's economic
class changes. An agent who achieves $c_i = 1$ from a group with
$\bar{c}_{g_k}$ near zero retains the markers of that group---the
``mimic man'' of the title. It is this decoupling that makes the
marker a soft signal rather than a hard one: because $m_i$ is not
generated by $c_i$, there is no likelihood $P(m \mid c)$ to condition
on, and Jeffrey conditioning over the group partition is forced.
...
The base model establishes the closure trap as a stable equilibrium
of bilateral Jeffrey updating, identifies marker salience $s_g$ and
group prior $\bar{c}_g$ as the two sufficient statistics governing
its behaviour, and characterises the saddle-node bifurcation through
which intergenerational mobility above a critical threshold
$\gamma^*$ eliminates the trap. Encounters are treated as bilateral
throughout---the environment in which they take place is taken as
given. In many real settings, however, that environment is itself
produced by a market actor: a restaurant calibrates its atmosphere
to attract preferred compositions, a school sets admission criteria,
a neighbourhood configures itself through pricing and zoning.

This section embeds bilateral Jeffrey updating within such a
platform---a venue, neighbourhood, or institution that mediates
encounters through an atmosphere vector and an admission policy. The
extension produces four results absent from the bilateral setting: a
...
The mechanism extends from persons to objects without additional
formal apparatus, because both occupy positions in the same sign
system grounded in economic history. \citet{Douglas1966} establishes
that pollution and disgust are not reactions to intrinsic properties
but to categorical position: dirt is ``matter out of place'' in a
classification system, and the disgust response functions to police
categorical boundaries. A cultural object---hilsa curry, chicken
feet, fermented soybean---carries a low $\bar{c}_g$ in the host
culture's prior; it is ``out of place'' when it appears in a
high-$\bar{c}_g$ context, and the disgust label attaches to its
categorical position, not to any intrinsic quality. What the Jeffrey
framework adds to Douglas is the persistence mechanism: under
Jeffrey conditioning, a positive individual encounter shifts
$P'(g_k)$ for \emph{this dish} but does not update the group prior $\bar{c}_g$ for
the cuisine as a category. Douglas describes the classification
system; the rigidity condition provides the information-theoretic
microfoundation for why individual reclassification fails to revise
it. A steady-state emerges in which a positive
...
individual consumption has normalised. Elites are more likely to
...
travel and specialist restaurant patronage). The food cycles through
three zones---disgusting, elite marker, mass-market---without the
ambient prior completing a full revision. The platform extension
sharpens this: restaurant platforms and food delivery apps curate
exposure through atmosphere instruments that encode the cuisine's
$\bar{c}_g$ into pricing, presentation, and neighbourhood placement,
so the three-zone cycle is not spontaneous but actively managed by
profit-maximising intermediaries.
...
These instruments operate on the contact channel rather than the
...
social information---coded language in reviews, word of mouth in
networks, selective social media signals among insiders.
Proposition~\ref{prop:shadow} characterises the shadow market that
arises in this contact channel as the legal regime tightens on the
contracting channel. The two channels require different instruments:
anti-discrimination law remains necessary on the contract side, and
the contact channel calls for mobility investment as the model
develops in Section~\ref{subsec:welfare}.
...
model is strictly more general. In the platform setting, these
bilateral encounters are mediated at scale: rideshare and delivery
platforms assign drivers to recipients through algorithms that strip
context, producing exactly the discordant-signal encounters the
model describes, while the platform's rating system creates a new
atmosphere instrument through which the shadow market operates
(Proposition~\ref{prop:shadow}).
...
the admission policy $\sigma$ is constrained, the shadow market intensifies
...
above its bifurcation threshold $\gamma^*$. This is not a normative verdict on those who chose
...
structurally false on both counts. The platform extension captures
the institutional dimension: colonial administrations, schools, and
civil services function as platforms whose atmosphere vectors
(language of instruction, dress codes, examination systems) were
calibrated to produce tokenism at institutional scale
(Proposition~\ref{prop:tokenism})---visible inclusion of compliant
individuals without revision of the group prior that governed
