...
in the value function at a reference point. The reference point is the
...
One of the most robust patterns in financial intermediation is the asymmetry between
boom and bust in the dynamics of \emph{dealer balance sheets}. Expansions ---
periods when aggregate dealer wealth sits above its long-run growth trend ---
are characterised by rapid balance sheet growth, large position sizes, elevated
volatility, and turbulent price movements. Contractions --- periods of
below-trend dealer wealth --- are slow, compressed, and grinding. This paper
is specifically concerned with this dealer-level phenomenon: the asymmetry
in the conditional variance of detrended dealer balance sheet deviations,
and its structural source in dealer loss aversion and economy-wide trend growth.
...
The pattern is well described, and GARCH-type volatility clustering has
been derived from diverse agent beliefs in rational belief equilibrium
models \citep{brock1998,follmer2005} and from endogenous uncertainty
propagated through belief heterogeneity \citep{kurz2001}. These
frameworks generate volatility persistence from the belief channel, and
some produce asymmetric responses to shocks. What they do not deliver
is the specific \emph{structural identification} that this paper targets:
the result that expansion variance exceeds contraction variance by a
...
mapping ($\beta \approx 1/(1+g)^2$), such that the two deep parameters
are independently recoverable from observed volatility dynamics.
The present paper identifies the minimal preference-based structure
that delivers this asymmetry and its orthogonal persistence as
equilibrium results.

The distinction between a statistical description and a structural explanation
matters beyond intellectual precision. Without a structural link between the
asymmetry and economic primitives, the pattern remains a calibration input ---
it can be fitted but not traced to its source. Regulators cannot design
instruments that target the source of the asymmetry, stress tests cannot be
calibrated to the structural variance ratio rather than to historical averages,
and cross-country fragility cannot be assessed from economic fundamentals.
With the structural link derived in this paper, the asymmetry becomes
actionable: the separation property gives regulatory and monetary policy
instruments structurally distinct targets, and the economic primitives
$\lambda$ and $g$ become estimable objects whose cross-country variation
predicts financial stability outcomes from first principles.
...
the reference point is a shared social trend growth benchmark rather than
...
Bayesian approaches with regime uncertainty \citep{veronesi1999,hamilton1989}
explore a different channel through which boom-bust asymmetry arises: agents
learn about regime probabilities and respond differently as posterior beliefs
shift, generating asymmetric volatility through belief dynamics. This paper
explores a complementary channel --- the preference channel. Even when agents
are certain about their positions, as dealers in liquid markets are (both
agents observe $D_{t-1}$ relative to the trend growth path in real time),
loss aversion generates boom-bust asymmetry through the price-setting
mechanism. The two channels are not mutually exclusive; both are plausibly
present in real markets. The contribution here is to isolate the preference
channel cleanly and derive its exact structural consequences.

The structural consequence of isolating the preference channel is the
separation property. The trend growth rate $g$ is publicly known
from national accounts data and enters the model as a deterministic
mean-reversion rate --- not inferred from signals, not subject to belief
updating. This is why $g$ enters only $\beta$ and $\lambda$ enters only
$|\gamma|/\alpha$, with no belief updating parameters present in either
channel. A model that also included Bayesian learning about $g$ would
generate additional volatility through the belief channel, making $\beta$
depend on both $g$ and agents' belief precision, and the clean separation
would require additional restrictions to restore. The preference channel
and the belief channel are therefore complementary objects of study:
this paper characterises what the preference channel delivers in isolation;
the interaction of the two channels is a direction for future work.
...
The reference point is the economy-wide common trend growth benchmark, growing
...
to catching-up-with-the-Joneses models in asset pricing \citep{abel1990,campbell1999}. The present implementation is distinct from these
...
\medskip\noindent\textbf{Why loss aversion rather than information asymmetry.}
In the market microstructure literature, boom-bust asymmetry can in principle
arise through two channels: a belief channel, in which agents process
information differently across regimes, and a preference channel, in which
agents respond differently to publicly observable states because of
asymmetric preferences. Both channels are plausibly present in real markets.
This paper isolates the preference channel for three reasons.

First, in the liquid exchange-traded settings this paper targets, the primary
source of price impact asymmetry is dealer risk management behaviour ---
VaR constraints and balance sheet benchmarks --- rather than private
information \citep{adrian2014,li2025}. Loss aversion operating through
pre-committed risk limits captures this institutional mechanism directly.

Second, loss aversion provides a more empirically tractable identification
target. The VaR-limit mechanism is directly observable in regulatory data
\citep{li2025}, whereas adverse selection must be inferred indirectly from
decompositions of the bid-ask spread or the permanent component of price
impact. The separation property derived below --- $|\gamma|/\alpha$
depending only on $\lambda$, $\beta$ depending only on $g$ --- would not
survive if both channels were simultaneously active without additional
identifying restrictions (Robustness Choice~10 confirms that restoring
adverse selection scales $\alpha$ and $|\gamma|$ by a common factor,
preserving their ratio but entangling the level with an additional
parameter $\delta$). Isolating the preference channel first is therefore
the correct sequencing for a structural identification programme.

Third, the cancellation result (Section~\ref{sec:cancellation}) establishes
precisely what the preference channel does and does not do: it does not
distort the informational content of news --- the return innovation is
regime-invariant --- but it does distort the \emph{wealth response} to
news, which is $\lambda^2$ times larger in expansion than in contraction.
The preference and belief channels are therefore complementary and
non-redundant objects of study; their interaction is a direction for
future work.
...
The second is the trend growth path itself, which provides the
...
The paper makes five contributions. The first is the derivation of
...
with the two parameters orthogonal. This separation provides exact
within-model identification of dealer loss aversion $\lambda$ from the
asymmetry ratio $|\gamma|/\alpha$, and of trend growth $g$ from the
persistence coefficient $\beta$, without external instruments, exclusion
restrictions, or auxiliary calibration targets. No other moment of the
data is needed and no cross-parameter restrictions are imposed from
outside the model. This contrasts with reduced-form GARCH estimation,
where $\alpha$, $\gamma$, and $\beta$ are freely varying and separating
$\lambda$ from $g$ requires auxiliary assumptions about the source of
persistence; with rational belief equilibrium approaches, where
identification requires distributional assumptions on agent beliefs; and
with prospect theory calibration, where $\lambda$ is identified against
external macroeconomic moments that must be taken as given
(Table~\ref{tab:identification_comparison}). The third is the IGARCH boundary
...
with measures of dealer loss aversion, with zero cross-effects. The fifth
is empirical confirmation of both structural channels using US dealer
balance sheet data: the variance asymmetry and persistence predictions
are each confirmed under the theoretically appropriate detrending method,
the pre-GFC asymmetry ratio is stable across growth regimes, and a
post-GFC regulatory structural break is explained by a one-parameter
extension that preserves the separation property.
...
Rational belief equilibrium \newline (Kurz \& Motolese 2001)
  ...
  & Distribution of belief types across agents
    (unobservable); updating rule specification;
    panel data on individual positions or structural
    restrictions on belief distribution \\[6pt]
Identification through \newline heteroskedasticity \newline (Rigobon 2003)
  & Structural shock variances
  & Exclusion restrictions: certain variables must
    affect volatility in one regime but not another;
    external knowledge of regime-driving variables \\[6pt]
...
Cross-country estimation \newline (empirical direction of this paper)
  & $\lambda_i$ and $g_i$ jointly
  & External $\lambda_i$ estimates (Wang et al.\ 2017);
    external $g_i$ from Penn World Table TFP data \\[6pt]
\textbf{This paper} \newline (separation property)
  & $\lambda$ from $|\gamma|/\alpha$; \newline $g$ from $\beta$
  & \textbf{Within-model only.} No external instruments,
    exclusion restrictions, or auxiliary calibration
    targets. Identification is exact and orthogonal:
...
setting driven by dealer loss aversion and the trend growth rate.
...
\begin{itemize}
  \item \textbf{Market maker (MM).} Posts the price impact schedule
    ...
    against the shared trend growth benchmark. Acts as Stackelberg leader.
...
uncertainty-driven. Both agents observe the trend growth path and the
...
\begin{remark}[Kyle price-impact structure without adverse selection]
\label{rem:kyle_scope}
The paper retains the Kyle (1985) linear price-impact structure
$p_t = p_{t-1} + \lambda_p(m_t + u_t)$ while shutting down the adverse
selection channel that motivates it in Kyle's original framework. In
Kyle (1985), the market maker prices against an informed counterparty:
$\lambda_p$ emerges from a zero-profit condition that compensates the
market maker for losses to informed traders. Here, the speculator's
signal $\mu_t$ is retained but the price impact schedule $\lambda_p$ is
set through pre-committed risk limits (Section~\ref{sec:mm_problem}),
not through a zero-profit condition against informed order flow.

This is a deliberate scope restriction, not an oversight. In liquid
exchange-traded markets --- the setting this paper targets --- the
dominant source of price-impact variation is dealer risk management
(VaR limits, capital constraints, balance sheet capacity), not adverse
selection against privately informed counterparties. The Kyle linear
price-impact specification is retained as a tractable reduced form for
the relationship between order flow and price; the market-making profit
$\lambda_p\sigma^2_u$ captures the bid-ask spread income that
compensates the dealer for providing liquidity, not for adverse
selection losses specifically. Noise traders are retained as the
standard device for sustaining a market-making equilibrium: without
uninformed order flow, the dealer would not post a schedule at all.
The adverse selection channel is the complementary object of study;
Robustness Choice~10 confirms that restoring it scales $\alpha$ and
$|\gamma|$ by a common factor but preserves their ratio.
\end{remark}
...
The economy grows at a constant constant trend growth rate $g > 0$. The shared
reference point is:
\begin{equation}
...
The trend growth path serves two distinct roles. Its level
...
transfers---the income $\lambda^{\rm eff}_p u^2_t$ that accrues to the
MM from uninformed order flow---are assumed to exit the system at each
date via consumption or external hedging demands. Under this assumption,
the net wealth change from the informed bilateral game satisfies
...
rather than noise trader welfare \citep{kyle1985,admati1988}. It
excludes settings where noise trader losses feed back into dealer
balance sheets through intermediation channels---such feedback is an
important extension but outside the scope of the present paper.
Under Assumption~\ref{ass:zerosum}, the bilateral zero-sum condition
holds exactly, and Proposition~\ref{prop:regime_separation} follows
as stated.
...
Three channels operate: (i) trend deflation $D_{t-1}/(1+g)$
...
\begin{equation}
  \pi^{MM}(\lambda_p)
  = \lambda_p\sigma^2_u - \frac{\sigma^2_\mu}{4\lambda_p}
  \label{eq:mm_profit}
\end{equation}
The unconstrained maximum of \eqref{eq:mm_profit} coincides with the
Kyle zero-profit benchmark $\lambda^0_p = \sigma_\mu/(2\sigma_u)$
regardless of regime. This is the standard cancellation result for
piecewise linear loss-averse preferences: the loss aversion scalar
$\lambda$ multiplies the objective in the loss region but does not
change the location of the interior optimum.

\begin{remark}[Why the kink does not bite in the interior]
\label{rem:kink_interior}
For a market maker with a piecewise linear value function---$v(\pi) =
\pi$ in the gain region and $v(\pi) = \lambda\pi$ in the loss
region---the first-order condition for $\lambda_p$ in each region is
$\partial\pi^{MM}/\partial\lambda_p = 0$ scaled by $1$ or $\lambda$
respectively. In both cases the optimum is the same interior point
$\lambda^0_p$: the scalar cancels. Loss aversion in the objective
alone therefore generates no equilibrium asymmetry in price impact.
For the kink to produce a behavioural effect it must enter through a
binding constraint on the strategy space---not through an unconstrained
interior optimisation. This motivates the risk-limit formulation below.
\end{remark}

\noindent\textbf{Risk limits as pre-committed loss-averse constraints.}
Consistent with \citet{adrian2014} and the desk-level evidence in
\citet{li2025}, we model the MM's loss aversion as operating through
pre-committed, regime-contingent risk limits on the price impact
schedule it posts. These limits are set at a lower frequency than
individual trading periods---reflecting the institutional reality that
VaR limits and balance sheet benchmarks are set by risk management
and updated infrequently---and within each period the MM posts the most
competitive $\lambda_p$ consistent with its pre-committed limit.

Formally, the MM's loss aversion parameter $\lambda > 1$ calibrates
two symmetric risk limits around the Kyle benchmark in log-space:
\begin{equation}
  \lambda^{\rm eff}_p =
  \begin{cases}
    \lambda^0_p/\lambda
      & D_{t-1} \geq 0 \quad\text{(expansion: MM in loss region,
        limit acts as a floor)} \\[4pt]
    \lambda\lambda^0_p
      & D_{t-1} < 0 \quad\text{(contraction: MM in gain region,
        limit acts as a cap)}
  \end{cases}
  \label{eq:risk_limits}
\end{equation}
The two limits are log-symmetric around $\lambda^0_p$:
$\log(\lambda\lambda^0_p) - \log\lambda^0_p =
\log\lambda^0_p - \log(\lambda^0_p/\lambda) = \log\lambda$.
This symmetry encodes loss aversion directly: the MM accepts a
$1/\lambda$ discount on price impact when recovering toward the
benchmark, and imposes a $\lambda$ premium when protecting accumulated
gains. Both limits bind in the equilibrium we study: the expansion
limit binds because the MM in the loss region is willing to sacrifice
margin to attract order flow; the contraction limit binds because the
MM in the gain region is protective of accumulated wealth and willing
to post a wide spread to reduce position exposure.

\begin{remark}[Institutional grounding of risk limits]
\label{rem:varlimits}
\citet{li2025} document, using granular desk-level regulatory data on
US Treasury dealers, that dealers reduce position changes and require
higher compensation as they approach their internal VaR limits. This
is precisely the contraction regime of equation~\eqref{eq:risk_limits}:
$\lambda^{\rm eff}_p = \lambda\lambda^0_p$ implies high price impact
and small speculator positions. \citet{adrian2014} document that
aggregate dealer leverage is procyclical and VaR-constrained,
consistent with the expansion regime: $\lambda^{\rm eff}_p =
\lambda^0_p/\lambda$ implies low price impact and large positions,
the structural signature of balance sheet expansion. The risk-limit
formulation in equation~\eqref{eq:risk_limits} is therefore the formal
analogue of the institutional behaviour documented in these studies.
\end{remark}
...
position relative to the trend growth benchmark --- not from differential
...
across the trend growth threshold. This is the model's formalisation of
...
\begin{equation}
  ...
  \label{eq:tgarch}
\end{equation}
with exact structural identifications for $\alpha$, $\gamma$, $\omega$
and a leading-order identification for $\beta$:
\begin{align}
  ...
  \beta  &\approx \frac{1}{(1+g)^2} \in (0,1)
    \quad \text{for } g > 0 \quad \text{(leading-order; see Proposition~\ref{prop:linearisation})}
    \label{eq:beta} \\
...
\begin{proof}
The exact part ($\alpha$, $\gamma$, $\omega$) follows from
Proposition~\ref{prop:var_asym}: for each realised regime $r$,
$V^r_t = (B^r)^2\Sigma[H_0 + H_2(\varepsilon^r_t)^2]$.
...
is bounded below 6\% for moderate deviations (Proposition~\ref{prop:linearisation}).
Signs and the $\beta$ limit follow from direct differentiation.
Full derivation in Proposition~\ref{prop:linearisation}.
\end{proof}
...
$g > 0$. All expressions are verified symbolically.
\qed
\end{proof}
...
\]
Both are verified symbolically. The persistence
parameter $\beta$ is therefore a sufficient statistic for the
trend growth rate $g$, and the asymmetry ratio
...
its intended application. The trend growth path $\bar{r}_t$ plays the role
...
eventually revert to the trend growth benchmark, while the transient
...
at the trend growth path crossing $D_{t-1} = 0$, estimating $\beta$ from
...
stagnant economy. Second, $\Phi_{\max}$ is decreasing in
...
\subsection{Empirical grounding of the variance ratio}
\label{sec:calibration_empirical}

The model predicts that expansion-regime variance in detrended dealer balance
sheet deviations exceeds contraction-regime variance by a factor of $\lambda^4$.
At the Tversky--Kahneman value $\lambda = 2.25$ this is a factor of $25.6$.
A natural question is whether this is consistent with observed data on dealer
balance sheet dynamics.

\citet{adrian2010} report that the standard deviation of quarterly changes in
primary dealer repo positions is approximately $2$--$3$ times larger in
expansion periods (above-trend leverage) than in contraction periods. This
implies an empirical variance ratio in the range $4$--$9$, corresponding to
$\lambda^4 \in [4,9]$ and thus $\lambda \in [1.41, 1.73]$. \citet{brunnermeier2009}
document that dealer balance sheet volatility roughly doubles between normal
and stressed periods, consistent with a variance ratio of approximately $4$
($\lambda \approx 1.41$). These estimates lie substantially below the
Tversky--Kahneman $\lambda = 2.25$, which is calibrated to individual
experimental choices rather than market-level dealer behaviour.

The implied structural $\lambda$ from market data therefore clusters in the
range $1.4$--$1.8$. This is consistent with the model's identification strategy:
$\lambda$ estimated from the asymmetry ratio $|\gamma|/\alpha =
(\lambda^4-1)/\lambda^4$ on dealer balance sheet data should produce
\emph{lower} values than laboratory experiments, because institutional risk
management partially attenuates the raw behavioural parameter. Table~\ref{tab:lambda_implied}
reports the implied $\lambda$ for a range of empirically plausible variance
ratios.

\begin{table}[ht]
\centering
\caption{Implied structural $\lambda$ from empirical variance ratios.
$(B^+)^2/(B^-)^2 = \lambda^4$; solved for $\lambda$.}
...
\noindent\footnotesize{\textit{Note:} Variance ratios of 4--9 are consistent
with \citet{adrian2010} and \citet{brunnermeier2009} estimates for dealer
balance sheet dynamics. The Tversky--Kahneman $\lambda = 2.25$ implies a
variance ratio of 25.6, which exceeds typical dealer-level estimates and may
reflect partial institutional attenuation of individual loss aversion in
market settings. The asymmetry ratio $|\gamma|/\alpha$ is invariant to $g$
and $\lambda^0_p$, making it robust to cross-market differences in liquidity
and growth.}
\end{table}
...
feature of the trend growth structure alone.
...
shared trend growth reference point (which, together with zero-sum dynamics and
...
This section provides empirical evidence on the model's structural
predictions using publicly available data on US dealer balance sheets
(Federal Reserve Z.1 Financial Accounts, Table L.130, Security Brokers
and Dealers, quarterly, 1952Q3--2025Q4). Leverage is defined as total
financial assets divided by proprietors' equity (book values).
...
Two structural features are jointly necessary for the boom-bust
asymmetry to emerge as an equilibrium theorem. First, the combination
...
aversion parameter. Second, the trend growth path provides the
mean-reversion that pins volatility persistence to the economy's long-run
growth rate $g$ through the leading-order mapping $\beta \approx 1/(1+g)^2$.
...
of observed wealth volatility dynamics, without external instruments or
exclusion restrictions beyond the model's maintained assumptions.

Empirical analysis using US dealer balance sheet data
(Federal Reserve Z.1 L.130, 1952--2025) provides five findings
supporting the model's predictions. First, expansion variance exceeds
contraction variance ($\hat{\mathcal{R}} = 2.07$, $p < 0.00001$), with
implied $\hat{\lambda} = 1.20$. Second, under deterministic detrending,
the persistence coefficient is $\hat{\phi} = 0.994$ against a model
prediction of $0.998$ --- a discrepancy of 0.4 percentage points,
compared to 33 percentage points under HP filtering. Third, the pre-GFC
asymmetry ratio is stable across growth regimes ($0.42$--$0.45$
despite a threefold difference in trend TFP growth), consistent with
the separation property. Fourth, the post-GFC sign reversal is
explained by a one-parameter regulatory floor extension that preserves
the separation property: regulation changed the asymmetry channel
while leaving persistence unchanged. Fifth, the secular stagnation
period ($g \approx 0.3\%$) exhibits near-unit-root persistence
($\hat{\phi} = 1.016$), consistent with the IGARCH boundary prediction.
The identification of $\lambda$ and $g$ requires different detrending
methods --- HP filtering for the variance ratio (local reference point)
and deterministic detrending for persistence (secular growth rate) ---
each theoretically grounded in the model's structure.
...
with threshold at the trend growth path crossing $D_{t-1} = 0$. The
...
The separation property --- $\partial\beta/\partial\lambda = 0$ and
$\partial(|\gamma|/\alpha)/\partial g = 0$ --- has three structural
implications that follow directly from the model's exact mappings,
independent of empirical calibration. All three are structural inferences
from the model rather than empirical findings; testing whether $\beta$
co-varies with trend TFP growth and $|\gamma|/\alpha$ co-varies with
experimental loss aversion estimates across markets --- with zero
cross-effects --- is the paper's primary empirical direction.

\noindent\textbf{A structural estimator for market-level loss aversion.}
The separation property provides a structural estimator for dealer loss
aversion from market data. Once $g$ is measured from trend TFP growth
and $\beta$ is estimated from the persistence of dealer wealth volatility,
the asymmetry ratio $|\gamma|/\alpha = (\lambda^4-1)/\lambda^4$ identifies
$\lambda$ without external instruments, exclusion restrictions, or
auxiliary calibration targets --- the identification is exact within the
model and requires no distributional assumptions on agent beliefs or
signal structure beyond the model's maintained assumptions
(Table~\ref{tab:identification_comparison}). Cross-country variation in
the estimated $\lambda$ could then be compared to experimental estimates
from \citet{wang2017} as an out-of-sample validation of the experimental
findings in a market context --- connecting the laboratory evidence on
loss aversion to its structural consequences in dealer markets.

\noindent\textbf{The IGARCH boundary as a structural early warning indicator.}
As $g \to 0$, $\beta \to 1$: the qualitative character of wealth volatility
dynamics changes from stationary to integrated. This is a mathematical
consequence of the exact mapping $\beta = 1/(1+g)^2$, not an empirical
claim. Monitoring $\hat{\beta}$ estimates from dealer balance sheet data
against trend TFP growth provides a structural early warning indicator:
when estimated persistence approaches one, the model says the economy's
trend growth rate is approaching zero --- a qualitative change in the
character of volatility dynamics, not a temporary shock. This reframes
financial stability monitoring from a backward-looking exercise (what
happened) to a forward-looking structural assessment (what the growth
rate implies for the persistence of future shocks). The IGARCH boundary
is therefore not a statistical nuisance to be modelled around but a
structural regime indicator with direct macroeconomic interpretation.

\noindent\textbf{Independent calibration of macroprudential and growth policy.}
Because $\beta = f(g)$ only and $|\gamma|/\alpha = f(\lambda)$ only,
the two policy optimisation problems are structurally independent.
The optimal effective $\lambda$ --- balancing dealer caution against
boom-bust amplitude, set through VaR limits or capital requirements ---
can be chosen without affecting the optimal $g$, and vice versa. This
provides a structural basis for independent calibration of macroprudential
and growth policies without cross-instrument spillovers: tightening
macroprudential constraints reduces $|\gamma|/\alpha$ without changing
$\beta$, and growth-enhancing reforms reduce $\beta$ without changing
$|\gamma|/\alpha$. Neither instrument substitutes for the other and
neither contaminates the other's target. This independence is a
consequence of the model's structure --- it holds exactly given the
maintained assumptions --- rather than an approximation or a policy
recommendation. The normative question of what the optimal $\lambda$ and
$g$ should be requires a welfare function the model does not specify;
what the model delivers is the structural result that the two choices
are orthogonal by construction.

Reducing effective $\lambda$ through macroprudential constraints is a
Pareto improvement \emph{within the model under its maintained
assumptions}: the speculator is unaffected
(loss aversion cancels in the first-order condition), the market maker
is protected from excessive loss aversion, and the real economy benefits
from more stable credit and faster post-crisis recovery. This result
relies on the bilateral zero-sum structure (Assumption~\ref{ass:zerosum})
and the pre-committed risk-limit specification; in richer models where
regulatory constraints interact with credit supply channels or dealer
entry decisions, the welfare implications may differ. The objective
is not to eliminate the asymmetry --- some boom-bust asymmetry reflects
rational intermediary caution and the high-variance expansion that
enables price discovery --- but to moderate excessive asymmetry. The
attainable equilibrium within the model combines high $g$ with moderate
$\lambda$: growth policy reducing persistence and macroprudential policy
moderating asymmetry, each operating on the structural parameter it
determines.
...
Third, the reference point construction --- a common trend growth path
...
following a cyclical growth process is the most important extension:
