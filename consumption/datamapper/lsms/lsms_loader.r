...
lsms_loader<-function(fu,ln,lgc) {
  ...
  load_market_prices <-function(year,dirprefix,fu,ln,use_pieces,aggregation_code){
    ...
    if (year ==2010) {
      ...
      a1 <- read.dta(paste(dirprefix,'/lsms/tnz2014/TZA_2014_NPS-R4_v03_M_STATA11/com_sec_cf.dta',sep=""),convert.factors = FALSE)
      a2 <- read.dta(paste(dirprefix,'/lsms/tnz2014/TZA_2014_NPS-R4_v03_M_v01_A_EXT_STATA11/com_sec_cf.dta',sep=""),convert.factors = FALSE)
      a <- rbind(a1,a2)
    ...
    if (year ==2014){
      if (F){
        ...
        cb <- fu()@get_translated_frame(dat=cbdat,
                                        names=ln()@ohs_seccb_columns_lsms(2014),
                                        m=ln()@ohs_seccb_mapping_lsms(2014))
        
        cb <- ln()@decode_clusterid(cb)
        cb$clusterid <- NULL
      ...
      a <- fu()@get_translated_frame(dat=adat,
                                     names=ln()@ohs_seca_columns_lsms(2014),
      ...
      b <- fu()@get_translated_frame(dat=bdat,
                                     names=ln()@ohs_info_columns_lsms_2014(),
                                     m=ln()@ohs_mapping_lsms_2014())
      ...
      j <- fu()@get_translated_frame(dat=jdat,
                                     ...
                                     m=ln()@get_lsms_secj_fields_mapping_2014())
  ...
  occupation_mapping<-function(){
    ...
    r=NULL;
    r=rbind(r,data.frame(occupation=14,occupation_rank=0,manual=F,agri=F))
    r=rbind(r,data.frame(occupation=13,occupation_rank=0,manual=F,agri=F))
    r=rbind(r,data.frame(occupation=12,occupation_rank=0,manual=NA,agri=F))
    r=rbind(r,data.frame(occupation=16,occupation_rank=0,manual=NA,agri=NA))
    r=rbind(r,data.frame(occupation=11,occupation_rank=0,manual=NA,agri=F))
    r=rbind(r,data.frame(occupation=1,occupation_rank=0,manual=T,agri=T))
    r=rbind(r,data.frame(occupation=17,occupation_rank=0,manual=NA,agri=NA))
    r=rbind(r,data.frame(occupation=2,occupation_rank=1,manual=T,agri=T))
    r=rbind(r,data.frame(occupation=3,occupation_rank=1,manual=T,agri=F))
    r=rbind(r,data.frame(occupation=4,occupation_rank=1,manual=F,agri=F))
    r=rbind(r,data.frame(occupation=7,occupation_rank=2,manual=F,agri=F))
    r=rbind(r,data.frame(occupation=9,occupation_rank=2,manual=F,agri=F))
    r=rbind(r,data.frame(occupation=10,occupation_rank=2,manual=F,agri=F))
    r=rbind(r,data.frame(occupation=15,occupation_rank=2,manual=NA,agri=NA))
    
    r=rbind(r,data.frame(occupation=8,occupation_rank=3,manual=F,agri=F))
    r=rbind(r,data.frame(occupation=5,occupation_rank=3,manual=F,agri=T))
    r=rbind(r,data.frame(occupation=6,occupation_rank=3,manual=F,agri=T))
  ...
  load_income_file<-function (year,dirprefix,fu,ln){
    ...
    if (year == 2014){
      idat1 <-read_tnz(paste(dirprefix,'./lsms/./tnz2014/TZA_2014_NPS-R4_v03_M_STATA11/hh_sec_e.dta',sep=""),FALSE,hhidColName = "y4_hhid")
      idat2 <-read_tnz(paste(dirprefix,'./lsms/./tnz2014/TZA_2014_NPS-R4_v03_M_v01_A_EXT_STATA11/hh_sec_e.dta',sep=""),FALSE,hhidColName = "y4_hhid")
      idat <- rbind(idat1,idat2) 
  ...
  add_market_price_to_fooddiary <- function (lgc,ld,marketpricesdata,ohsdata,ddata){
    ...
    for (nrow in seq(1,dim(subst)[1])){
      ...
      toadd  <- subset(prices,shortname==efrom)
      if (dim(toadd)[1]>0){
        print(paste("add_market_price_to_fooddiary -prices for", eto, "with prices from",efrom))
        prices <- rbind(prices,toadd %>% mutate(shortname=eto))
      }
    }
  ...
  add_market_price_to_misc_diary <-function (curyear,dirprefix,fu,ln, groups,lgc,ld,marketpricesdata,ohsdata,ddata) {
    ...
    for (yr in years_to_use) {
      ...
      if (dim(mdat)[1]>0) {
        ...
        mdat$year    <- yr
        print("add_market_price_to_misc_diary - Ignoring the use of village column")
        mdat$village <- NULL
    ...
    if (dim(paddedalldat)[1]>0){
      paddedalldat$curyear <- rep(curyear,dim(paddedalldat)[1])
      print("test:")
      print(head(paddedalldat))
      curprices <- ddply(paddedalldat[,c("region","district","shortname","category","year","price","curyear")],
                         .(region,district,category,shortname),summarise, 
                         price=lgc()@fill_missing_yearvals(category,year,price,curyear))
      paddedalldat$cyr <- NULL
      ...
      print(paste("add_market_price_to_misc_diary - No price inference for items :",(dim(k)[1]-dim(diary)[1]),"/",dim(k)[1],"(",
                  100*(dim(k)[1]-dim(diary)[1])/dim(k)[1],"%)"))
    ...
    if (setequal(groups$group,c("quality"))) {
      ...
      if ( use_market_prices == FALSE) {
        ...
        if (is.null(hhpm)) {
          ...
          print(paste("group_collect - Obtained interpolation/other-data for items:",toString(unique(hhpm$shortname))))
          hhp <- rbind(hhp,hhpm)
        }
