...
calculateROE<- function(){
  ...
  for ( file in files){
    ...
    # MVE->WACC
    mve_input = ts.intersect(tsshares,tsltdebt,marketprices)
    mve_input =  as.data.frame(mve_input);
    E = mve_input$tsshares* mve_input$marketprices.MidPrice
    D =mve_input$tsltdebt;
    V = E+D;
