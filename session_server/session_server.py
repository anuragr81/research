...
def sync_refresh(request_data,fs,sess) :  
   print "type(request_data)=",type(request_data), " request_data: ",request_data
...
def accept_logon(request_data,fs,sess):
  ...
  for entry in parsed_request_data:
    if 'user' in entry.keys() and 'password' in entry.keys() : 
...
class MTServer (ThreadingMixIn, BaseHTTPServer.HTTPServer):
