...
#define css_title_row "<tr \"header\"><td \"left\">Function</td><td>Calls</td><td>MCycles</td><td>Avg</td><td>Self MCycles</td><td \"right\">Self Avg</td></tr>\n"
#define css_totals_row "<tr \"header\"><td \"left\">Function</td><td>Calls</td><td>Self MCycles</td><td \"right\">Self Avg</td></tr>\n"
   	 ...
   	 */
   	 ...
   	 struct FormatHtml {
   		 ...
   		 void operator()( Caller *item ) const {
   			 fprintf( mFile, "\t<tr %s><td><table \"tree\"><tr>", !item->GetParent() ? "\"" css_thread_style "\"": "\"h\"" );
   			 ...
   			 if ( !item->GetParent() )
   				 fprintf( mFile, "<td \"text\">%s</td></tr></table></td><td \"number\">%u</td><td \"number\">%0.4f (%3.0f%%)</td><td \"number\">%0.4f</td><td \"number\">%0.4f</td><td \"number\">%0.4f</td></tr>\n", 
   					 item->mName, 
   					 item->mTimer.calls,
   					 ms,
   					 globalpct,
   				 ...
   				 fprintf( mFile, "<td \"text\">%s</td></tr></table></td><td \"number\" \"background-color:%s\">%u</td><td \"number\" \"background-color:%s\">%0.4f (%3.0f%%)</td><td \"number\" \"background-color:%s\">%0.6f</td><td \"number\" \"background-color:%s\">%0.4f</td><td \"number\" \"background-color:%s\">%0.6f</td></tr>\n",
   					 item->mName, 
   	 ...
   	 struct FormatHtmlTop {
   		 ...
   		 void operator()( Caller *item, bool islast ) const {
   			 ...
   			 if ( !item->GetParent() ) {
   				 fprintf( mFile, "<td \"text\">%s</td></tr></table></td><td \"number\">%u</td><td \"number\">%0.4f (%.0f%%)</td><td \"number\">%0.4f</td></tr>\n", 
   					 item->mName, 
   					 item->mTimer.calls,
   					 ms,
   					 globalpct,
   				 ...
   				 fprintf( mFile, "<td \"text\">%s</td></tr></table></td><td \"number\" \"background-color:%s\">%u</td><td \"number\" \"background-color:%s\">%0.4f (%.0f%%)</td><td \"number\" \"background-color:%s\">%0.4f</td></tr>\n", 
   					 item->mName, 
   					 maxStats.color( Max::Calls, item->mTimer.calls ),  item->mTimer.calls,
   					 maxStats.color( Max::Ms, ms ), ms,
   					 globalpct,
    ...
    */
    ...
    struct HTMLDumper {
   	 void Init() {
   		 ...
   		 strftime( timeFormat, 255, "%Y%m%d_%H%M%S.%s", now_tm );
   	 ...
   	 void GlobalInfo( u64 rawCycles ) {
   		 fputs( "<div \"overall\"><table>", f );
   		 if ( programName ) {
   			 fprintf( f, "<tr><td \"title\">Command Line: </td><td>%s", programName );
   			 if ( commandLine )
   				 fprintf( f, " %s", commandLine );
   			 fputs( "</td></tr>", f );
   		 }
   		 fprintf( f, "<tr><td \"title\">Date: </td><td>%s</td></tr><tr><td \"title\">Raw run time: </td><td>%.2f mcycles</td></tr>\n", 
   			 timeFormat, Timer::ms( rawCycles ) );
   	 }

   	 void ThreadsInfo( u64 totalCalls, f64 timerOverhead, f64 rdtscOverhead ) {
   		 fprintf( f, "<tr><td \"title\">Total calls: </td><td>" PRINTFU64() "</td></tr>\n", totalCalls );
   		 fprintf( f, "<tr><td \"title\">rdtsc overhead: </td><td>%.0f cycles</td></tr>\n", rdtscOverhead );
   		 fprintf( f, "<tr><td \"title\">Per call overhead: </td><td>%.0f cycles</td></tr>\n", timerOverhead );
   		 fprintf( f, "<tr><td \"title\">Estimated overhead: </td><td>%.4f mcycles</td></tr>\n", Timer::ms( totalCalls * timerOverhead ) );
   		 fprintf( f, "</table></div>\n" );
   	 }

   	 void PrintThread( Caller *root ) {
   		 fputs( "<div \"thread\"><table>\n", f );
   	 ...
   	 void PrintAccumulated( Caller *accumulated ) {
   		 ...
   		 fputs( "<tr \"" css_thread_style "\"><td><table><tr><td><img src=\"img/root.gif\" /></td><td>Functions sorted by self time</td></tr></table></td><td></td><td></td><td></td></tr>\n", f );
...
class AGF:
   def __init__(self,threshExt,timeLimit,exceedHigh):
	self.firstvisit=True;
	self.threshExt = threshExt;
	self.timeLimit = timeLimit
	self.exceedHigh = exceedHigh;
       
   def process(self,T,X,skip):
   	if self.firstvisit:
      	self.X = X # memory of 1
      	self.XF = X # memory of 1
      	self.lowX = X
      	self.highX = X
      	self.highT = T
      	self.firstvisit=False
       	...
       	if skip:
         	...
         	if X <= self.XF : # X(ii) < XF(ii-1)
            	# case 2
            	self.XF = X	# XF(ii) = X(ii)
            	self.X = X
            	if X < self.highX/self.threshExt or X < self.lowX :
                 	self.lowX = X
         	else :
                 	if X > self.X :
                     	# case 3
                    	if (T - self.highT) < self.timeLimit and self.highX > self.lowX*self.threshExt and X < self.exceedHigh*self.highX :
                        	self.X = X  # notice that XF value is retained/unchanged - this is equivalent to XF(ii) = XF(ii-1)
                    	else:
                        	self.XF = X; # XF(ii) = X(ii)
                        	self.X = X;
                    	if X > self.lowX * self.threshExt or X > self.highX:
                        	self.highX = X
                        	self.highT = T
                 	else:
                     	# case 4
...
def loadCsv(filename):
	...
	with open(filename) as ff:
  	...
  	for row in ffreader:
     	retlist.append(int(row[0]))
	return retlist


agf = AGF(timeLimit=5e+8,threshExt=1.1,exceedHigh=1.05)

#agf.process(T=41583425535634,X=
#agf.process(T=41583425535634,X=29,skip=0)
#agf.process(T=41588106662604,X=29,skip=0)

T= loadCsv("T.csv");
X= loadCsv("X.csv");
maskskip= loadCsv("maskskip.csv");
